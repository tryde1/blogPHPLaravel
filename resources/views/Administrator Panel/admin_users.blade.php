            ...
            @foreach($users as $user)
                <div class="user">
                    <form action="{{ route('find.Action') }}" method="post" class="form">
                        ...
                        <input type="email" value="{{ $user['email'] }}" class="email" name="email" readonly
                               placeholder="email">
                        <input type="text" value="{{ $user['name'] }}" class="name" name="name" readonly
                               placeholder="name">
                        <input type="text" value="{{ $user['surname'] }}" class="surname" name="surname" readonly
                               placeholder="surname">
                        <input type="text" value="{{ $user['phonenumber'] }}" class="phonenumber" name="phonenumber"
                               readonly placeholder="phonenumber">
                        <input type="text" value="{{ $user['created_at'] }}" class="create_date" name="create_date"
                               readonly placeholder="create_date">
                        <input type="text" value="{{ $user['updated_at'] }}" class="update_date" name="update_date"
                               readonly placeholder="update_date">
                        <select name="permissions">
                            <option value="user" @if($user['permissions'] == 'user') selected @endif >User</option>
                            <option value="moderator" @if($user['permissions'] == 'moderator') selected @endif>
                                Moderator
                            </option>
